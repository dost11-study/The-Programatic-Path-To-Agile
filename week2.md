...
3. 실력이 부족해 불안함을 느끼는 경우 -> 실력 높이기
	...
	2. 도구적 접근
		1. 디버거, 자동 통합 도구, 코드 분석툴, REPL 환경 등 사용
		2. 오픈 소스 라이브러리 빌려쓰기
	3. 내관적 접근
		1. 비슷한 일을 했던 경험을 머릿속에서 되살려 보기
	4. 난이도 낮추기
		1. 아기버전(0.0.1 버전) 즉 MVP부터 시작해보기
		2. 알고리즘을 푸는 경우 알고리즘 자체만 신경쓰도록 파이썬으로 풀고 C로 풀기 (파이썬으로 풀고 C로 포팅함으로서 난이도 낮추기)

하지만 자신의 실력이나 작업의 난이도가 조금씩 요동 치면, 현재 자신의 업무 처리 속도가 외부적으로 문제가 되지 않는 범위 내에서 적절하게 난이도와 실력을 조정해야한다.

메타인지 전략은 공부를 잘하는 사람들의 중요한 특징 중 하나로 꼽는다. 공부를 넘어서서 모든 분야의 전문성에 있어서도 메타인지는 핵심적인 요소이다.

### 팀장이 할 수 있는 일

팀장들에게는 팀원의 상태를 파악하고 그들이 몰입으로 가게 도와주는 것 자체가 의도적 수련이된다.